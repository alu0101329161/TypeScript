...
// import {NoteType} from "../P11/peticiontype";
...
export class ManejarNotas {
  ...
  listarNotas(usuario: string) {
    ...
    fs.readdirSync(`BaseDatosNotas/${usuario}`).length > 0) {
      ...
      console.log(chalk.green(`Notas del usuario: ${usuario} listadas correctamente`));
      return Notas;
    } else {
      console.log(chalk.red(`Notas del usuario: ${usuario} no encontradas`));
  ...
  leerNota(usuario: string, titulo: string) {
    if (fs.existsSync(`BaseDatosNotas/${usuario}/${titulo}.json`)) {
      ...
      console.log(chalk.green(`Notas del usuario: ${usuario} encontrada`));

      return nota;
    } else {
      console.log(chalk.red(`Notas del usuario: ${usuario} no encontrada`));
